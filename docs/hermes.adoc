...
In Hermes all keyword extraction methods implement the _KeywordExtactor_ interface which is an _Extractor_ with the addition of a `fit(corpus)` method. The `fit` method is used when corpus level statistics are needed in order to perform keyword extraction or when keyword extraction is performed in a supervised manner.
