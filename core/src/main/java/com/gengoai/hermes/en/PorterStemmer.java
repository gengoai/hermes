...
/**
 * Stemmer, implementing the Porter Stemming Algorithm The Stemmer class transforms a word into its root form.  The
 * input word can be provided a character at time (by calling add()), or at once by calling one of the various
...
public final class PorterStemmer {
   ...
   */
   ...
   /**
    * Stem the word placed into the Stemmer buffer through calls to add(). Returns true if the stemming process resulted
    * in a word different from the input.  You can retrieve the result with getResultLength()/getResultBuffer() or
    * toString().
    */
   public void stem() {
      k = i - 1;
