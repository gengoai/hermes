 ...
 */
public abstract class IOBTrainer extends CommandLineApplication {
    ...
    */
   public IOBTrainer(String name, AnnotationType annotationType) {
      super(name);
   ...
   protected ExampleDataset getDataset(FeatureExtractor<HString> featurizer) throws IOException {
      ...
      return collection.asSequenceDataset(s -> {
         ...
         s.labelGenerator(new IOBLabelMaker(trainingAnnotation, validTags()));
      });
   }
   ...
   protected void label() throws Exception {
      IOBTagger tagger = IOBTagger.read(model);
   ...
   protected void split() throws Exception {
      ...
      if(preprocessors != null && preprocessors.size() > 0) {
         ...
         IOBTagger tagger = new IOBTagger(featurizer, annotationType, labeler, version);
         IOBEvaluation eval = new IOBEvaluation();
